...
export class Migration1741958043604 implements MigrationInterface {
    name = 'Migration1741958043604'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`Event_Particpations\` (\`user_id\` int NOT NULL, \`event_id\` int NOT NULL, \`date_time_participation\` datetime NOT NULL, \`win\` tinyint NOT NULL DEFAULT 0, \`score\` int NOT NULL DEFAULT '0', PRIMARY KEY (\`user_id\`, \`event_id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`Events\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(100) NOT NULL, \`content\` longtext NOT NULL, \`is_restricted\` tinyint NOT NULL DEFAULT 0, \`suspended\` tinyint NOT NULL DEFAULT 0, \`deleted\` tinyint NOT NULL DEFAULT 0, \`event_type\` enum ('morpion', 'motus', 'sondage') NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`manager_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`Saved_Ressources\` (\`user_id\` int NOT NULL, \`ressource_id\` int NOT NULL, \`is_favorite\` tinyint NOT NULL DEFAULT 0, \`is_to_later\` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (\`user_id\`, \`ressource_id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`Consulted_Ressources\` (\`user_id\` int NOT NULL, \`ressource_id\` int NOT NULL, \`date_time_consult\` datetime NOT NULL, \`like\` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (\`user_id\`, \`ressource_id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`User_Report\` (\`reporter_id\` int NOT NULL, \`reported_user_id\` int NOT NULL, \`content\` longtext NOT NULL, \`moderator_view\` longtext NOT NULL, \`is_resolved\` tinyint NOT NULL DEFAULT 0, \`ressource_type\` enum ('superAdmin', 'admin', 'moderator', 'user', 'visitor') NOT NULL DEFAULT 'user', \`reported_comment_id\` int NULL, PRIMARY KEY (\`reporter_id\`, \`reported_user_id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`Users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` varchar(100) NULL, \`email\` varchar(100) NOT NULL, \`username\` varchar(60) NOT NULL, \`password\` varchar(255) NOT NULL, \`banned\` tinyint NOT NULL DEFAULT 0, \`disabled\` tinyint NOT NULL DEFAULT 0, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`role\` enum ('superAdmin', 'admin', 'moderator', 'user', 'visitor') NOT NULL DEFAULT 'user', UNIQUE INDEX \`IDX_b8cdbf0507991260838b4521f1\` (\`uuid\`), UNIQUE INDEX \`IDX_3c3ab3f49a87e6ddb607f3c494\` (\`email\`), UNIQUE INDEX \`IDX_ffc81a3b97dcbf8e320d5106c0\` (\`username\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`Comments\` (\`id\` int NOT NULL AUTO_INCREMENT, \`message\` longtext NOT NULL, \`deleted\` tinyint NOT NULL DEFAULT 0, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`ressource_id\` int NULL, \`autor_id\` int NULL, \`parent_comment_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`Ressources\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(100) NOT NULL, \`content_text\` longtext NOT NULL, \`content_link\` varchar(100) NULL, \`admin_validation\` tinyint NOT NULL DEFAULT 0, \`date_time_validation\` datetime NOT NULL, \`ressource_type\` enum ('text', 'image', 'video', 'pdf') NOT NULL DEFAULT 'text', \`visibility\` enum ('restricted', 'public', 'private') NOT NULL DEFAULT 'private', \`state\` enum ('draft', 'toValidate', 'published', 'suspended', 'deleted') NOT NULL DEFAULT 'draft', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`category_id\` int NULL, \`creator_id\` int NULL, \`validator_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`Categories\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(50) NOT NULL, \`icon_path\` varchar(100) NOT NULL, \`color\` varchar(20) NOT NULL, \`deleted\` tinyint NOT NULL DEFAULT 0, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`last_autor_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`Event_Particpations\` ADD CONSTRAINT \`FK_92993aae24aa94c592685306bd0\` FOREIGN KEY (\`event_id\`) REFERENCES \`Events\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`Event_Particpations\` ADD CONSTRAINT \`FK_895b93995253d7df4f3bac2e15d\` FOREIGN KEY (\`user_id\`) REFERENCES \`Users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`Event_Particpations\` DROP FOREIGN KEY \`FK_895b93995253d7df4f3bac2e15d\``);
        await queryRunner.query(`ALTER TABLE \`Event_Particpations\` DROP FOREIGN KEY \`FK_92993aae24aa94c592685306bd0\``);
        ...
        await queryRunner.query(`DROP INDEX \`IDX_b8cdbf0507991260838b4521f1\` ON \`Users\``);
        ...
        await queryRunner.query(`DROP TABLE \`Event_Particpations\``);
    }
