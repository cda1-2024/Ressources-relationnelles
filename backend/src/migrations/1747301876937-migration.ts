...
export class Migration1747301876937 implements MigrationInterface {
    name = 'Migration1747301876937'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`Saved_Ressources\` (\`user_id\` uuid NOT NULL, \`ressource_id\` uuid NOT NULL, \`is_favorite\` tinyint NOT NULL DEFAULT 0, \`is_to_later\` tinyint NOT NULL DEFAULT 0, \`like\` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (\`user_id\`, \`ressource_id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`Users\` (\`id\` uuid NOT NULL, \`uuid_google\` varchar(100) NULL, \`email\` varchar(100) NOT NULL, \`username\` varchar(60) NOT NULL, \`bio\` varchar(1000) NOT NULL DEFAULT '', \`password\` varchar(255) NOT NULL, \`banned\` tinyint NOT NULL DEFAULT 0, \`disabled\` tinyint NOT NULL DEFAULT 0, \`city\` varchar(100) NOT NULL DEFAULT '', \`region\` varchar(100) NOT NULL DEFAULT '', \`country\` varchar(100) NOT NULL DEFAULT '', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`role\` enum ('superAdmin', 'admin', 'moderator', 'user', 'visitor') NOT NULL DEFAULT 'user', UNIQUE INDEX \`IDX_72c8c13cb0d4ecca0029e37ea3\` (\`uuid_google\`), UNIQUE INDEX \`IDX_3c3ab3f49a87e6ddb607f3c494\` (\`email\`), UNIQUE INDEX \`IDX_ffc81a3b97dcbf8e320d5106c0\` (\`username\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`Comments\` (\`id\` uuid NOT NULL, \`message\` longtext NOT NULL, \`deleted\` tinyint NOT NULL DEFAULT 0, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`ressource_id\` uuid NULL, \`author_id\` uuid NULL, \`parent_comment_id\` uuid NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`Comments\` ADD CONSTRAINT \`FK_e143682fe5432d08726c74a797f\` FOREIGN KEY (\`author_id\`) REFERENCES \`Users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`Comments\` DROP FOREIGN KEY \`FK_e143682fe5432d08726c74a797f\``);
