...
export class Migration1745566740069 implements MigrationInterface {
    name = 'Migration1745566740069'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`Event_Participations\` (\`user_id\` int NOT NULL, \`event_id\` int NOT NULL, \`date_time_participation\` datetime NOT NULL, \`win\` tinyint NOT NULL DEFAULT 0, \`score\` int NOT NULL DEFAULT '0', PRIMARY KEY (\`user_id\`, \`event_id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`Consulted_Ressources\` (\`user_id\` int NOT NULL, \`ressource_id\` int NOT NULL, \`date_time_consult\` datetime NOT NULL, PRIMARY KEY (\`user_id\`, \`ressource_id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`Users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` varchar(100) NULL, \`email\` varchar(100) NOT NULL, \`username\` varchar(60) NOT NULL, \`password\` varchar(255) NOT NULL, \`banned\` tinyint NOT NULL DEFAULT 0, \`disabled\` tinyint NOT NULL DEFAULT 0, \`city\` varchar(100) NOT NULL DEFAULT '', \`region\` varchar(100) NOT NULL DEFAULT '', \`country\` varchar(100) NOT NULL DEFAULT '', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`role\` enum ('superAdmin', 'admin', 'moderator', 'user', 'visitor') NOT NULL DEFAULT 'user', UNIQUE INDEX \`IDX_b8cdbf0507991260838b4521f1\` (\`uuid\`), UNIQUE INDEX \`IDX_3c3ab3f49a87e6ddb607f3c494\` (\`email\`), UNIQUE INDEX \`IDX_ffc81a3b97dcbf8e320d5106c0\` (\`username\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`Search_Stats\` (\`search\` varchar(255) NOT NULL, \`user_id\` int NOT NULL, \`date_time_search\` datetime NOT NULL, PRIMARY KEY (\`search\`, \`user_id\`)) ENGINE=InnoDB`);
